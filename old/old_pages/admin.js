...
const MsgSmall = styled.li`
  ...
  background: ${props => (!props.unread ? props.theme.blue : props.theme.light)};
`;
          ...
          <List simple>
            {messages.map(message => (
              <MsgSmall key={message._id} unread={message.read}>
                ...
                <small onClick={() => this.markAsUnread(message._id)}>mark as unread</small>
                <small onClick={() => this.markAsRead(message._id)}>mark as read</small>
              </MsgSmall>
            ))}
          </List>
          {activeMsg.firstname ? (
            <Message>
              ...
              <small>
                {Date(activeMsg.createdAt).replace(`GMT-0400 (Eastern Daylight Time)`, ``)}
              </small>
